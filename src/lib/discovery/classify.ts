import { ClassifiedResult, SearchResult } from "./types";
...
You are given public web search results gathered while searching for people or sites already commercially promoting the brand "${brand}" (domain: ${domain}).
...
3. Is there real evidence of an affiliate/referral/ambassador relationship: a personalized promo code, an affiliate/referral link, a disclosed partnership, or a dedicated commercial review with a purchase/referral call to action?
...
- Only set validCandidate: true when there is a personalized promo code, an affiliate/referral link, a disclosed ambassador/partner relationship, or a dedicated commercial review with a clear purchase/referral call to action.
...
- Never invent a name, platform, profile URL, promo code, or contact detail that is not visible in the title/snippet — use null if unknown.
...
export async function classifyResults(
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: "user", content: `Classify these ${results.length} search results:\n\n${numbered}` },
      ],
      tools: [
        {
          name: TOOL_NAME,
          description: "Report the classification of each search result.",
  ...
  return parsed.classifications
    ...
    .map((item): ClassifiedResult => {
      ...
      return {
        ...
        contact: typeof item.contact === "string" ? item.contact : null,
